            ...
            <div class="relative w-full h-full max-w-md md:h-auto">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 text-center">
                        ...
                        <form action={{ route('income.delete', ['id' => $income->id]) }} method="post"
                            class="inline-flex items-center">
                            @method('delete')
                            @csrf
                            <button data-modal-toggle="delete-modal-{{ $income->id }}" type="submit"
                        ...
                        <button data-modal-toggle="delete-modal-{{ $income->id }}" type="button"
                                    ...
                                    @foreach ($income_list as $income)
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            <td class="py-4 px-6 font-bold text-green-500">
                                                {{ 'Rp. ' . number_format($income->totalPemasukan) }}
                                            </td>
                                            <td class="py-4 px-6 font-bold text-red-500">
                                                {{ 'Rp. ' . number_format($income->hargaModal) }}
                                            </td>
                                            <td class="py-4 px-6">
                                                <a href={{ route('income.form.edit', ['id' => $income->id]) }}
                                                    class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                                                
                                                <button
                                                    class="font-medium text-red-600 dark:text-red-500 hover:underline ml-4"
                                                    type="button"
                                                    data-modal-toggle="delete-modal-{{ $income->id }}">Hapus</button>
                                            </td>
                                        </tr>
                                    @endforeach
                                @endforeach
